...
Below, total steps per day is displayed as a histogram. Mean value of the total number of steps taken per day (`r round(step_mean, 2)`) is highlighted by a vertical purple line, the median (`r step_median`) by a vertical orange line. In this original dataset, the mean is a bit to the left of the median.
...
Typically, the person is most active at `r max_activity_time`, that is, `r max_activity_index`th interval.


## Imputing missing values

The data set has `r sum(is.na(data))` rows with missing values (NAs).
...
The following histogram graphically shows the total number of steps, mean and median of the new dataset `imputedata`. The mean value of the total number of steps taken per day (`r imputestep_mean`) is highlighted by a vertical red line, the median (`r imputestep_median `) by a vertical green line. The mean and the median overlap, and the peak of days with no recorded steps is gone. Both values have increased compared to the original data set. The increase of the mean, however, is much stronger.
...
Due to imputation, the total sum of steps in these two months increases from `r sum(data$steps, na.rm=T)` to `r sum(imputedata$steps)`.
...
==================  
Created by Prachi Singh on `r date()`
